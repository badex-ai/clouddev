...
export async function createTask(taskData: CreateTask) {
  try {
    ...
    if (!response.ok) {
      const errorMessage = responseData.message || responseData.detail || 'Failed to create task';
...
export async function getTaskForDay(family_id: string, selectedDate: string) {
  try {
    ...
    if (!response.ok) {
      const errorMessage = responseData.message || responseData.detail || 'Failed to fetch task';
