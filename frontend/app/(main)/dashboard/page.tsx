...
function Dashboard() {
  ...
  const updateTaskInList = (updatedTask: Task) => {
    setTasks((prevTasks) =>
      prevTasks.map((task) => (task.public_id === updatedTask.public_id ? updatedTask : task))
    );
  };
    ...
    if (tasks?.length === 0) {
      taskTable = (
        <div className="w-full border rounded-sm border-[#888888] p-8 flex flex-col items-center justify-center ">
          ...
          <p className="text-gray-600 text-lg font-medium">You don't have any tasks</p>
        </div>
      );
    } else {
      taskTable = (
        ...
        <div className="flex items-center justify-between">
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-gray-800">
              {userData?.family?.name}'s Family Board
            </h1>
          ...
          <div className="flex items-center gap-3">
            <div className="bg-blue-100 px-4 py-2 rounded-lg border border-blue-200">
              <h2 className="text-xl font-semibold text-blue-800">{getDate()}</h2>
            </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 pt-4">
                <div className="space-y-2">
                  ...
                  <Input id="task-title" placeholder="Enter task title" {...register('title')} />
                  {errors.title && (
