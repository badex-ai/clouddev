...
async def get_user_route(
    req: UserRequest = Body(...), db: Session = Depends(get_db)
) -> dict:
...
def deactivate_family_member_route(user_id: str, db: Session = Depends(get_db)) -> None:
    logger.info(f"Deactivating user: {user_id}")
    deactivate_user(user_id, db)


@router.patch("/{user_id}/activate", status_code=status.HTTP_204_NO_CONTENT)
def reactivate_family_member_route(user_id: str, db: Session = Depends(get_db)) -> None:
