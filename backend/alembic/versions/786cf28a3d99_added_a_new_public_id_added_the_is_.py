...
def add_public_id_to_table(table_name: str):
    ...
    op.add_column(
        table_name,
        sa.Column(
            "public_id", sa.String(length=36), nullable=True, unique=True, index=True
        ),
    )
    ...
    for row in rows:
        conn.execute(
            sa.text(
                f"UPDATE {table_name} SET public_id = :pid, is_deleted = :deleted WHERE id = :id"
            ),
...
def upgrade() -> None:
    ...
    op.alter_column("tasks", "family_id", existing_type=sa.INTEGER(), nullable=False)
    op.alter_column(
        "tasks",
        "due_date",
    ...
    op.alter_column("users", "family_id", existing_type=sa.INTEGER(), nullable=False)
...
def downgrade() -> None:
    ...
    op.alter_column("users", "family_id", existing_type=sa.INTEGER(), nullable=True)
    ...
    op.alter_column("tasks", "family_id", existing_type=sa.INTEGER(), nullable=True)
