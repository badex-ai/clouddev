...
def upgrade() -> None:
    op.execute(
        "CREATE TYPE task_status_new AS ENUM('initialised', 'in-progress', 'completed')"
    )
...
def downgrade() -> None:
    op.execute(
        "CREATE TYPE task_status_old as ENUM('PENDING', 'IN_PROGRESS', 'COMPLETED')"
    )

    op.execute(
        "ALTER TABLE tasks ALTER COLUMN status TYPE task_status_old USING status::text::task_status_old "
    )
