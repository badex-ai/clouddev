...
class Task(Base):
    ...
    creator_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("users.public_id"), nullable=False
    )
    assignee_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("users.public_id"), nullable=False
    )
    family_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("families.public_id"), nullable=False
    )
        ...
        if checklist is not None:
            ...
            for i, item in enumerate(checklist):
                ...
                if not all(field in item for field in required_fields):
                    raise ValueError(
                        f"Checklist item {i} missing required fields: {required_fields}"
                    )

                if not isinstance(item["completed"], bool):
                    raise ValueError(
                        f"Checklist item {i} 'completed' must be a boolean"
                    )

                if (
                    not isinstance(item["title"], str)
                    or len(item["title"].strip()) == 0
                ):
                    raise ValueError(
                        f"Checklist item {i} 'title' must be a non-empty string"
                    )
    ...
    def add_checklist_item(self, item_id: str, title: str, completed: bool = False):
        ...
        new_item = {"id": item_id, "title": title.strip(), "completed": completed}
    ...
    def update_checklist_item(
        self, item_id: str, title: str = None, completed: bool = None
    ):
...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(
        SQLEnum(UserRole, name="userrole"), default=UserRole.member, nullable=False
    )
